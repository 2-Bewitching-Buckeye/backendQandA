...
const rl = readline.createInterface({
  input: fs.createReadStream('csvFiles/answers.csv'),
  output: fs.createWriteStream('cleanFiles/answers_clean.csv'),
  // dirty: fs.createWriteStream('flaggedFiles/answers_flagged.csv'),
  crflDelay: Infinity
})

rl.flaggedFiles =  fs.createWriteStream('./flaggedFiles/answers_flagged.csv');
...
rl.on('line', (line) => {
  ...
  if (isNaN(Number(columns[1]))) {
    rl.flaggedFiles.write(`${line}\n`);
    return};
  if (columns[2].length < 4 || columns[2].length > 1000) {
    rl.flaggedFiles.write(`${line}\n`);
    return};
